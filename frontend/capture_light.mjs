...
async function captureLightMode() {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(ARTIFACT_DIR, 'sdlc_meetings_dashboard_light_verified.png') });
    console.log('Saved sdlc_meetings_dashboard_light_verified.png');
  } catch (err) {
    console.error(err);
  } finally {
    await browser.close();
