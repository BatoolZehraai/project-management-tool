...
export default function TimelinePlanner({
  ...
  const sortedMonthTasks = [...tasksInCurrentMonth].sort((a, b) => (a.due_date || '').localeCompare(b.due_date || ''));

  return (
    <div className="space-y-4 max-w-7xl mx-auto">
      {/* 1. Master Header Strip */}
      <div
        className={`p-3.5 sm:p-5 rounded-2xl border transition-all flex flex-col xl:flex-row items-start xl:items-center justify-between gap-3.5 ${
          isDarkMode
      ...
      >
        ...
        <div className="flex items-center space-x-3">
          <div
            className={`p-2 sm:p-2.5 rounded-xl border shrink-0 transition-transform ${
              isDarkMode
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h2 className={`text-sm sm:text-base font-extrabold tracking-tight ${isDarkMode ? 'text-zinc-100' : 'text-slate-900'}`}>
            ...
            <p className={`text-[11px] sm:text-[11.5px] mt-0.5 ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
              Scheduled task deliverables, sprint deadlines, and governance milestone tracker
            </p>
          </div>
        </div>

        {/* Right: Controls Strip (View Toggle, Stage Filter, Month Navigator, Today, New Task) */}
        <div className="flex flex-wrap items-center gap-2 w-full xl:w-auto justify-between xl:justify-end">
          {/* View Toggle (Calendar / Agenda) */}
          <div
      ...
      {/* 3. Main View Area (Calendar Grid or Agenda Stream) */}
      {viewMode === 'calendar' ? (
        <div
            ...
            {days.map((d, i) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[120px] sm:min-h-[135px] p-2 sm:p-2.5 flex flex-col justify-between transition-all group relative border-r last:border-r-0 ${
                  ...
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div
                      className={`transition-all ${
                        isToday
                          ? 'w-6.5 h-6.5 sm:w-7 sm:h-7 rounded-lg border-2 border-purple-400 bg-gradient-to-tr from-purple-600 to-indigo-600 text-white font-extrabold text-xs flex items-center justify-center shadow-md shadow-purple-500/40 ring-2 ring-purple-400/30'
                          : d.isCurrentMonth
                            ? isDarkMode
                              ? 'w-6 h-6 sm:w-6.5 sm:h-6.5 rounded-md border border-zinc-700 bg-zinc-850 text-zinc-200 font-bold text-xs flex items-center justify-center group-hover:border-purple-400 group-hover:text-purple-300 group-hover:bg-purple-950/40 shadow-xs'
                              : 'w-6 h-6 sm:w-6.5 sm:h-6.5 rounded-md border border-slate-300 bg-slate-100 text-slate-800 font-bold text-xs flex items-center justify-center group-hover:border-violet-500 group-hover:text-violet-700 group-hover:bg-purple-50 shadow-xs'
                            : 'w-5.5 h-5.5 rounded-md border border-zinc-800/40 text-zinc-600 text-xs flex items-center justify-center font-medium'
                      }`}
                    >
                      <span>{d.dayNum}</span>
                    </div>
        ...
        /* Agenda List View (Mobile-Friendly Linear Timeline) */
        <div
        ...
        >
          <div className="flex items-center justify-between pb-2 border-b border-inherit">
            <h3 className={`font-extrabold text-xs uppercase tracking-wider ${isDarkMode ? 'text-zinc-200' : 'text-slate-800'}`}>
              Agenda for {monthName}
            </h3>
            <span className="text-xs font-mono font-bold text-purple-400">
              {sortedMonthTasks.length} total scheduled
            </span>
          </div>

          {sortedMonthTasks.length === 0 ? (
            <div className="py-12 text-center text-zinc-500 text-xs font-medium space-y-1">
              <p>No deliverables scheduled for {monthName}.</p>
              <p className="text-[10px] opacity-70">Click 'New Task' above to add a milestone.</p>
            </div>
          ) : (
            <div className="space-y-2.5">
              {sortedMonthTasks.map(t => {
                const isCompleted = t.status === 'Completed';
                const isCritical = t.priority === 'Critical' || t.priority === 'High';
                const taskPhase = phases.find(p => p.id === t.phase_id);
                ...
                return (
                  <div
                    ...
                    className={`p-3.5 rounded-xl border transition-all cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
                      isDarkMode
                        ? 'bg-[#151726]/90 border-zinc-800 hover:border-purple-500/50 hover:bg-[#1a1d30]'
                  ...
                  >
                    <div className="flex items-start space-x-3 min-w-0">
                      <div className="mt-0.5 shrink-0">
                        {isCompleted ? (
                          ...
                          <span className="w-2.5 h-2.5 rounded-full bg-rose-500 block animate-pulse mt-1" />
                        ) : (
                          <span className="w-2.5 h-2.5 rounded-full bg-purple-400 block mt-1" />
                        )}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                          <h4 className={`font-bold text-xs leading-snug ${isCompleted ? 'line-through opacity-75' : ''} ${isDarkMode ? 'text-zinc-100' : 'text-slate-900'}`}>
                            {t.title}
                          </h4>
                    ...
                    <div className="flex items-center justify-between sm:justify-end space-x-3 text-xs shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-inherit">
                      {t.assignee_name && (
                        <div className="flex items-center space-x-1.5">
                          ...
                          <span className={`text-[11px] truncate max-w-[90px] ${isDarkMode ? 'text-zinc-300' : 'text-slate-600'}`}>
                            {t.assignee_name}
                          </span>
                        </div>
                      )}
                      <span className={`font-mono font-bold text-[11px] px-2 py-0.5 rounded-md border ${
                        isOverdue(t.due_date) && !isCompleted
